...
declare @col int = (select min(ColID) from #grid)
declare @row int = (select min(RowID) from #grid)
drop type if exists AOCrange
go
create type AOCrange as table (ID int, Height int)
go
...
as
begin
  if (select count(*) from @seq) <= 1  
	return 0 --break: we are at a border

  declare @result int = 0 
  declare @orgHeight int = (select top(1) Height from @seq order by ID)
  declare @heightMatchedFlag bit = 0
  declare @p int = (select min(ID) from @seq)
  set @p = (select min(ID) from @seq where ID > @p) --start comparison one three out
  while @p is not null and @heightMatchedFlag = 0
	begin
		if @orgHeight <= (select Height from @seq where ID = @p)
			set @heightMatchedFlag = 1
				
		set @result = @result + 1
		set @p = (select min(ID) from @seq where ID > @p)
	end
  return @result
end
go
...
begin
	set @Row = (select min(RowID) from #grid where ColID = @Col)
	...
	begin
		...
		insert into @AOCrange (ID, Height)
		select ID = row_number() over (order by ColID desc),Height from #grid where RowID = @Row and ColID <= @Col
		select @VisibleR = @VisibleR * dbo.AOCVisibleRange(@AOCrange)

		--move right
		delete from @AOCrange
		insert into @AOCrange (ID, Height)
		select ID = row_number() over (order by ColID asc),Height from #grid where RowID = @Row and ColID >= @Col
		select @VisibleR = @VisibleR * dbo.AOCVisibleRange(@AOCrange)
		
		--move up
		delete from @AOCrange
		insert into @AOCrange (ID, Height)
		select ID = row_number() over (order by RowID desc),Height from #grid where RowID <= @Row and ColID = @Col
		select @VisibleR = @VisibleR * dbo.AOCVisibleRange(@AOCrange)
		
		--move down
		delete from @AOCrange
		insert into @AOCrange (ID, Height)
		select ID = row_number() over (order by RowID asc),Height from #grid where RowID >= @Row and ColID = @Col
		select @VisibleR = @VisibleR * dbo.AOCVisibleRange(@AOCrange)

		update #grid set VisibleRange = @VisibleR where ColID = @Col and RowID = @Row

		set @Row = (select min(RowID) from #grid where RowID > @Row and ColID = @Col)
	end
	set @Col = (select min(ColID) from #grid where ColID > @Col)
end
